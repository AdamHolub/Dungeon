...
const enemyHPendfinal = document.getElementById("enemyHPendfinal");
...
attack.onmousedown = () => {
  if (enemyHPendfinal.innerHTML > 0 && PlayerHPcount > 0) {
    player.style.left = `${50}%`;
    enemyHPendfinal.innerHTML -= countOfDmg;
  }
  if (enemyHPendfinal.innerHTML <= 0) {
    info.innerHTML = "Info: Zabil si enemy!";
    clearInterval(interv);
    pomucka--;
  }
  if (enemyHPendfinal.innerHTML <= 0 && pomucka == 0) {
    countOfCoins += 1000;
    PlayerCoins.innerHTML = `Coins: ${countOfCoins}`;
    parseInt(sessionStorage.setItem("coiny", countOfCoins));
    enemyHPendfinal.innerHTML = 0;
  }
};
attack.onmouseup = () => {
  if (PlayerHPcount > 0) {
